...
<html>
	...
	<!-------------------- Cart ADD  ----------------------------------------------->
			<button class="topBtn" onclick="addShop()";><span>Add Item</span></button>
			<script>
				function addShop(){
...
<!-------------------- Cart   ----------------------------------------------->
			<a href="cart.php"><button id="favTab"class="topBtn" ><span><i class="fas fa-shopping-cart" style="margin-right: 5px;"></i>Cart</span></button></a>
<!-------------------- Cart END  ----------------------------------------------->
	...
	<!--------------------------- Shop Başlangıç ---------------------------------------------->
				<div class="items">
					...
					$stmt=$pdo->prepare("SELECT * FROM items WHERE item_category= ? ");
					...
					<div class="card" >
					  ...
					  <div class="cartmiktar">
					  	<button style="border:none;" onclick="miktarazalt(<?=$item['item_id']?>);"><i class=" fa fa-chevron-circle-down" aria-hidden="true"></i></button>
					  	<p id="<?=$item['item_id']?>" style="margin-top: 10px;"> 1 </p>
					  	<button style="border:none;" onclick="miktarartir(<?=$item['item_id']?>);"><i class=" fa fa-chevron-circle-up" aria-hidden="true"></i></button>
					  </div>
